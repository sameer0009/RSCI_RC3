...
export default function ProblemDetailPage() {
  ...
  const getVerdictColor = (verdict: string) => {
    switch (verdict) {
      ...
      case 'WrongAnswer': return 'text-red-600 dark:text-red-400';
      case 'TimeLimitExceeded': return 'text-yellow-600 dark:text-yellow-400';
      case 'RuntimeError':
      case 'CompilationError': return 'text-orange-600 dark:text-orange-400';
          ...
          <div className="flex border-b border-gray-200 dark:border-[#2d2d2d] bg-gray-50 dark:bg-[#1e1e1e] px-2 h-12 items-center">
            <button
              ...
              className={\`px-4 py-2 text-sm font-semibold border-b-2 transition-colors \${
                activeLeftTab === 'description' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
              }\`}
            >
              Description
            </button>
            <button
              ...
              className={\`px-4 py-2 text-sm font-semibold border-b-2 transition-colors \${
                activeLeftTab === 'solutions' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
              }\`}
            >
              Editorial & Solutions
            </button>
              ...
              <>
                ...
                <div className="flex gap-2 mb-6">
                  ...
                  {problem.topics.map((topic) => (
                    <span key={topic} className="px-3 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full">{topic}</span>
                  ))}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition">+ Post Solution</button>
                  </div>
            ...
            <Panel defaultSize={showOutput ? 60 : 100} minSize={20} className="flex flex-col relative">
              <div className="p-2 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#1e1e1e] flex justify-between items-center h-12">
                ...
                <div className="flex gap-2">
                  <button onClick={handleRunCode} disabled={running || submitting || testingSamples} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-gray-200 dark:bg-[#2d2d2d] text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-[#3d3d3d] disabled:opacity-50 transition-colors">
                    <Terminal size={14} /> {running ? 'Running...' : 'Run'}
                  </button>
                  <button onClick={handleTestSamples} disabled={running || submitting || testingSamples} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-gray-200 dark:bg-[#2d2d2d] text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-[#3d3d3d] disabled:opacity-50 transition-colors">
                    <Play size={14} className="text-green-500" /> {testingSamples ? 'Testing...' : 'Test'}
                  </button>
                  <button onClick={handleSubmit} disabled={running || submitting || testingSamples} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-primary-600 text-white rounded-md hover:bg-primary-700 disabled:opacity-50 transition-colors">
              ...
              <div className="flex-1 w-full bg-[#1e1e1e]">
                <Editor
                  ...
                  options={{ minimap: { enabled: false }, fontSize: 14, lineNumbers: 'on', scrollBeyondLastLine: false, automaticLayout: true, padding: { top: 16, bottom: 16 }, fontFamily: "'JetBrains Mono', 'Fira Code', 'Consolas', monospace", fontLigatures: true, renderWhitespace: "selection" }}
                />
              </div>
            </Panel>
              ...
              <>
                ...
                <Panel defaultSize={40} minSize={10} className="flex flex-col bg-gray-50 dark:bg-[#1a1a1a]">
                  <div className="flex border-b border-gray-200 dark:border-[#2d2d2d] bg-gray-100 dark:bg-[#1a1a1a]">
                    <button onClick={() => setActiveTab('custom')} className={\`px-4 py-2 text-sm font-medium border-b-2 transition-colors \${activeTab === 'custom' ? 'border-primary-500 text-primary-500 dark:text-primary-400' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}\`}>Custom Run</button>
                    <button onClick={() => setActiveTab('samples')} className={\`px-4 py-2 text-sm font-medium border-b-2 transition-colors \${activeTab === 'samples' ? 'border-primary-500 text-primary-500 dark:text-primary-400' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}\`}>Sample Tests</button>
                    <button onClick={() => setActiveTab('submission')} className={\`px-4 py-2 text-sm font-medium border-b-2 transition-colors \${activeTab === 'submission' ? 'border-primary-500 text-primary-500 dark:text-primary-400' : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}\`}>Submission</button>
                    <div className="flex-1"></div>
                    <button onClick={() => setShowOutput(false)} className="px-3 py-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">×</button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    {activeTab === 'custom' && (
                      <div className="flex flex-col h-full gap-4">
                        <div className="flex-1">
                          <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Custom Input</label>
                          <textarea value={customInput} onChange={(e) => setCustomInput(e.target.value)} className="w-full h-24 px-3 py-2 border border-gray-300 dark:border-[#333] rounded-md bg-white dark:bg-[#1e1e1e] text-gray-900 dark:text-gray-200 text-sm font-mono focus:ring-1 focus:ring-primary-500 outline-none resize-y" placeholder="Type input here..." />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Output</label>
                    ...
                    {activeTab === 'samples' && (
                      <div className="space-y-4">
                        {sampleResults.length === 0 && !testingSamples && (
                          <div className="h-32 flex items-center justify-center text-gray-400 dark:text-gray-500 text-sm border-2 border-dashed border-gray-200 dark:border-[#333] rounded-xl">Click "Test" to evaluate your code against the samples.</div>
                        )}
                        {testingSamples && (
                          <div className="h-32 flex items-center justify-center space-x-2 text-primary-500 text-sm font-medium"><div className="w-4 h-4 border-2 border-primary-500 border-t-transparent rounded-full animate-spin"></div><span>Evaluating sample cases...</span></div>
                        )}
                        {sampleResults.map((result, index) => (
                          <div key={result.testCaseId} className={\`p-4 border rounded-xl overflow-hidden transition-colors \${result.verdict === 'Accepted' ? 'bg-green-50/50 border-green-200 dark:bg-green-900/10 dark:border-green-900/30' : 'bg-red-50/50 border-red-200 dark:bg-red-900/10 dark:border-red-900/30'}\`}>
                            <div className="flex justify-between items-center mb-3">
                              <span className="font-semibold text-gray-900 dark:text-white">Example {index + 1}</span>
                              <div className="flex items-center gap-3">
                                <span className={\`text-sm font-bold tracking-wide \${getVerdictColor(result.verdict)}\`}>{result.verdict.toUpperCase()}</span>
                                <span className="px-2 py-1 bg-white/50 dark:bg-black/20 rounded text-xs text-gray-600 dark:text-gray-400 font-medium">{result.points}/{result.maxPoints} pts</span>
                              </div>
                            </div>
                              ...
                              <div className="col-span-2">
                                {result.verdict !== 'Accepted' && result.output !== undefined && (
                                  <div className="mb-3">
                                    ...
                                    <pre className="mt-1 p-2 bg-white dark:bg-black/40 rounded border border-red-100 dark:border-red-900/30 text-sm font-mono break-all whitespace-pre-wrap text-red-600 dark:text-red-400 font-medium">{result.output || '<Empty>'}</pre>
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            {result.errorMessage && (
                              <div className="mt-3">
                                ...
                                <pre className="mt-1 p-3 bg-red-100/50 dark:bg-red-900/20 rounded border border-red-200 dark:border-red-800 text-xs font-mono break-all whitespace-pre-wrap text-red-700 dark:text-red-400">{result.errorMessage}</pre>
                              </div>
                            )}
                          </div>
                        ))}
                      ...
                      <div className="h-full flex flex-col justify-center max-w-2xl mx-auto w-full space-y-6">
                        ...
                        {submissionResult && (
                          <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl border border-gray-200 dark:border-[#333] shadow-md overflow-hidden">
                            <div className={\`p-6 border-b \${submissionResult.verdict === 'Accepted' ? 'bg-green-50 border-green-100 dark:bg-green-900/20 dark:border-green-900/30' : 'bg-red-50 border-red-100 dark:bg-red-900/20 dark:border-red-900/30'}\`}>
                              <h2 className={\`text-3xl font-black tracking-tight \${getVerdictColor(submissionResult.verdict)}\`}>{submissionResult.verdict}</h2>
                              <p className="mt-1 text-sm font-medium text-gray-500 dark:text-gray-400">Submitted just now</p>
                            </div>
                            <div className="p-6 grid grid-cols-3 gap-6 text-center">
                              <div className="flex flex-col p-4 bg-gray-50 dark:bg-black/20 rounded-xl">
                                ...
                                <span className="text-2xl font-black text-gray-900 dark:text-white">{submissionResult.testCasesPassed}<span className="text-gray-400 dark:text-gray-600 text-xl font-bold">/{submissionResult.totalTestCases}</span></span>
                              </div>
                              <div className="flex flex-col p-4 bg-gray-50 dark:bg-black/20 rounded-xl">
                            ...
                            {submissionResult.error && (
                              <div className="mx-6 mb-6 p-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-xl">
                                ...
                                <pre className="text-xs text-red-700 dark:text-red-300 font-mono break-all whitespace-pre-wrap">{submissionResult.error}</pre>
                              </div>
                            )}
                          </div>
                        )}
...
const result = fs.writeFileSync('d:/Apps/RSCI-RC3/frontend/src/app/problems/[slug]/page.tsx', page_tsx_content)
