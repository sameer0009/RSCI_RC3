...
function EditAssignmentContent() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const assignmentRes = await api.get(`/classrooms/assignments/${assignmentId}`);
      const assignment = assignmentRes.data.data;

      setFormData({
        title: assignment.title,
        description: assignment.description || '',
        dueDate: new Date(assignment.dueDate).toISOString().slice(0, 16), // Format for datetime-local
      });
      setSelectedProblems(assignment.problems.map((p: any) => p.id));
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.put(`/classrooms/assignments/${assignmentId}`, {
        ...formData,
      ...
      alert(error.response?.data?.error?.message || 'Failed to update assignment');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <>
        <Navbar />
        <div className="min-h-screen bg-gray-50 dark:bg-dark-bg py-8 flex items-center justify-center">
          <div className="text-gray-500 animate-pulse">Loading assignment data...</div>
        </div>
      </>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-dark-bg py-8">
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-white dark:bg-dark-card rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div className="p-8">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit Assignment</h1>
